...
if ($_POST) {
    ...
    $dados['termo_adesao'] = ($_FILES['termo_adesao']['tmp_name'] ? $_FILES['termo_adesao'] : null);
    $dados['contrato'] = ($_FILES['contrato']['tmp_name'] ? $_FILES['contrato'] : null);
    
    $file_contrato = $_FILES['contrato'];
    $file_termo = $_FILES['termo_adesao'];
    $edital = new Edital();
    $edital->createEdital($dados);
    if (!$edital->getResult()):
        echo $edital->getMsg();
    else:
        $uploud = new Uploud();
        $uploud->File($file_contrato, 'editais/' .$dados['ano'].'/'.$tipo.'/'.$edital->getResult().'/');
        $uploud->File($file_termo);
        echo $edital->getMsg();
        unset($dados);
    endif;
}
...
<form name="form" method="post" action="" enctype="multipart/form-data">
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" name="descricao" id="descricao" class="form-control" placeholder="Digite a descrição da licitação" minlength="10" required>
        </div>
    </div>
    <div class="form-row">
    ...
    <div class="form-row">
        <div class="form-group col-6">
            ...
            <input name="termo_adesao" type="file" class="form-control" required />
        </div>
        <div class="form-group col-6">
            <label for="linkEdital">Contrato</label>
            <input name="contrato" type="file" class="form-control" required />
        </div>
    </div>
    <div class="form-group row">
